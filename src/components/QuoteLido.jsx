...
const FileUpload = ({
      ...
      if (account) {
        if (isChecked) {
          if (finalPrice >= 1) {
            ...
            try {
              ...
              if (response.status === 200) {
                const mint = await NFT_contract.methods
                  ...
                  .mintNFTCover_Lido(requestId, ratio, response.data.IpfsHash)
                  .send({
                    from: account,
                  });
            ...
            alert("The payment amount must be equal to or greater than 1 ETH.");
          }
        } else {
  ...
  const onScreengenerateImage = async () => {
    ...
    ctx.fillText(`Unstaking Cover`, 10, 60);
    ctx.fillText(`Token Type: ETH`, 10, 90);
    ctx.fillText(`Request ID: ${requestId}`, 10, 150);
    // ctx.fillText(`Cover Amount : ${amount} USDT`, 10, 180); // 커버 amount
    ctx.fillText(`Claimable Amount :  ${activePrice} USDT`, 10, 210); // 보험금 지급받을 수 있는 가격
    ctx.fillText(`Standard LINK Price : ${ethPrice} USDT`, 10, 240); // 민팅 당시의 토큰 가격
    ctx.fillText(`Expired Date : ${formatDate(future30Days)}`, 10, 270); // 만료 날짜 (만료 날짜가 지난 경우엔 컨트랙트에서 다른 uri 반환.)
    ctx.fillText(`Mint Date : ${formatDate(currentTime)}`, 10, 300); // 민팅 날짜
