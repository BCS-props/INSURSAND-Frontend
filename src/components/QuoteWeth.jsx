...
// flie 값 반환 (피나타 업로드용)
const onScreengenerateImage = async ({ amount, toUsdt }) => {
  console.log(amount, toUsdt);
  // 캔버스 생성
  ...
  ctx.fillText(`Expired Date : ${formatDate(future30Days)}`, 10, 120); // 만료 날짜 (만료 날짜가 지난 경우엔 컨트랙트에서 다른 uri 반환.)
  ctx.fillText(`Cover Amount : ${amount} DAI`, 10, 150); // 커버 amount
  ctx.fillText(`Cover Active Price :  $`, 10, 180); // 보험금 지급받을 수 있는 가격
  ctx.fillText(`Standard WETH Price : ${toUsdt} $`, 10, 210); // 민팅 당시의 토큰 가격
  ctx.fillText(`Mint Date : ${formatDate(currentTime)}`, 10, 240); // 민팅 날짜
